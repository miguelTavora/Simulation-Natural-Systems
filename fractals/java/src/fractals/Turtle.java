...
public class Turtle 
{
	...
	public void setPoseCenter(double[] position, float orientation) {
		float[] c = plot.getPixelCoord(position);
		p.translate(c[1], c[0]);
		p.pushMatrix();
		p.popMatrix();
	...
	public void renderColor(LSystem lsystem,int[]color) {
	...
	public void render(LSystemStochastic lsystem) {
		p.stroke(0);
		p.strokeWeight(1);
		for (int i = 0; i < lsystem.getSequence().length(); i++) {
			char c = lsystem.getSequence().charAt(i);
